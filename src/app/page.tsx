...
export default function HomePage() {
  ...
  const handleGenerate = useCallback(async (formData: FormData) => {
    ...
    try {
      ...
      body.append(
        ...
        String(formData.generateApplicationEmail)
      );
      ...
      body.append(
        ...
        String(formData.generateDirectMessage)
      );
  ...
  const handleResumeUpdate = useCallback(
    (updated: OptimizedResume, newPdf: string) => {
      setResult((prev) =>
        prev ? { ...prev, optimizedResume: updated, pdfBase64: newPdf } : prev
      );
    },
    []
  );
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-border/40 bg-card/30 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <a
              href="https://github.com/Leonel-H29/resume-tailor"
              target="_blank"
      ...
      <section className="pt-10 pb-8 px-4 sm:px-6 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 border border-accent/20 text-xs text-accent font-medium mb-5">
          <Zap size={11} /> Resume · Cover Letter · Answers · Email · DM — One
          Request
        </div>
        ...
        <p className="text-muted-foreground text-sm sm:text-base max-w-xl mx-auto">
          Upload your resume and a job description. AI optimises,
          keyword-matches, and optionally generates a cover letter, Q&A answers,
          an application email, and a recruiter DM — all in one request.
        </p>
      </section>
        ...
        {state === 'idle' && (
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              {
                label: 'ATS-Optimised',
                desc: 'Clean formatting that passes filters',
              },
              {
                label: 'Per-Output Language',
                desc: 'Each output in its own language',
              },
              {
                label: 'PDF Editor',
                desc: 'Edit generated resume before downloading',
              },
              {
                label: 'Recruiter DM',
                desc: '≤250-char LinkedIn message in one request',
              },
            ].map(({ label, desc }) => (
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
      ...
      <p className="text-sm text-muted-foreground max-w-xs">
        ...
        <span className="text-foreground font-medium">Generate</span>. Enable
        optional extras for a complete application package.
      </p>
